...
ERNO.IeCss3DRenderer = function(cube) {
	...
	var getObjectCSSTransform = function() {
		...
		return function(matrix) {
			...
			return 'translate3d(-50%,-50%,0) translate3d(' + epsilon(position.x) + 'px, ' + epsilon(position
					.y) + 'px, ' + epsilon(position.z) + 'px) ' +
				'rotateX(' + epsilon(euler.x) + 'rad) rotateY(' + epsilon(euler.y) + 'rad) rotateZ(' +
				epsilon(euler.z) + 'rad) ' +
	...
	this.render = function(scene, camera) {
		...
		for (var i = 0, l = renderList.length; i < l; i++) {
			...
			objWorldPosition.set(renderList[i].matrixWorld.elements[12], renderList[i].matrixWorld.elements[13],
			...
			if (renderList[i] instanceof THREE.CSS3DObject) renderList[i].element.style.zIndex = renderList[i]
				.userData.zIndex;
		}
		firstRender = false;
	};
