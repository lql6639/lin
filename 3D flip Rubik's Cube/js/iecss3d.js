...
ERNO.renderers.IeCSS3DCubelet.methods = function() {
	...
	return {
		getFaceElements: function(selector) {
			...
			return Array.prototype.slice.call(this.cube.domElement.querySelectorAll('.cubeletId-' + this.id +
				selectorString));
		},
		show: function() {
		...
		showIntroverts: function() {
			...
			return function(onlyAxis, soft) {
				...
				if (onlyAxis) {
					...
					only = (Math.abs(Math.round(axis.x)) === 1) ? '.axisX' : (Math.round(Math.abs(axis
						.y)) === 1) ? '.axisY' : '.axisZ';
				}
