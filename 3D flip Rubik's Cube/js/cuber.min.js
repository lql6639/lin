(function() {
	...
	THREE.CSS3DRenderer = function() {
		...
		var g = function(a) {
			...
			m = function() {
				...
				return function(d) {
					d.decompose(a, e, b);
					return "translate3d(-50%,-50%,0) translate3d(" + g(a.x) + "px, " + g(a.y) + "px, " + g(a
							.z) + "px) rotateX(" + g(c.x) + "rad) rotateY(" + g(c.y) +
						"rad) rotateZ(" + g(c.z) + "rad) scale3d(" + g(b.x) + ", " + g(-b.y) + ", " + g(b
							.z) + ")"
				}
			}(),
			s = function(a, b) {
				if (a instanceof THREE.CSS3DObject) {
					var c;
					a instanceof THREE.CSS3DSprite ? (e.copy(b.matrixWorldInverse), e.transpose(), e
						.copyPosition(a.matrixWorld), e.scale(a.scale), e.elements[3] = 0, e.elements[7] =
						0, e.elements[11] = 0, e.elements[15] = 1, c = m(e)) : c = m(a.matrixWorld);
					var d = a.element;
		...
		this.render = function(a, e) {
			...
			l = "matrix3d(" + g(l[0]) + "," + g(-l[1]) + "," + g(l[2]) + "," + g(l[3]) + "," + g(l[4]) +
				"," + g(-l[5]) + "," + g(l[6]) + "," + g(l[7]) + "," + g(l[8]) + "," + g(-l[9]) + "," + g(l[
					10]) + "," + g(l[11]) + "," + g(l[12]) + "," + g(-l[13]) + "," + g(l[14]) + "," + g(l[
					15]) + ")";
	...
	ERNO.extend(Number.prototype, {
		...
		rand: function(a) {
			var b;
			return void 0 !== a ? (b = Math.min(this, a), a = Math.max(this, a), b + Math.floor(Math
				.random() *
				(a - b))) : Math.floor(Math.random() * this)
		},
		random: function(a) {
			var b;
			return void 0 !== a ? (b = Math.min(this, a), a = Math.max(this, a), b + Math.random() * (
				a - b)) : Math.random() * this
		},
	...
	ERNO.extend(String.prototype, {
		...
		invert: function() {
			var a = "",
				b;
			for (b = 0; b < this.length; b++) a = this.charAt(b) === this.charAt(b).toUpperCase() ? a +
				this.charAt(b).toLowerCase() : a + this.charAt(b).toUpperCase();
			return a
		},
	...
	ERNO.extend(Array.prototype, {
		...
		shuffle: function() {
			var a = this.length,
				b, c, d;
			if (0 == a) return !1;
			for (; --a;) b = Math.floor(Math.random() * (a + 1)), c = this[a], d = this[b], this[a] = d,
				this[b] = c;
			return this
		},
		toArray: function() {
		...
		toHtml: function() {
			var a, b = "<ul>";
			for (a = 0; a < this.length; a++) b = this[a] instanceof Array ? b + this[a].toHtml() : b +
				("<li>" + this[a] + "</li>");
			return b + "</ul>"
		},
		toText: function(a) {
			...
			d = "";
			for (b =
				0; b < this.length; b++) d = this[b] instanceof Array ? d + (c + this[b].toText(a +
				1)) : d + (c + this[b]);
			return d
		}
	});
	...
	ERNO.Direction.getDirectionById = function(a) {
		return [ERNO.Direction.FRONT, ERNO.Direction.UP, ERNO.Direction.RIGHT, ERNO.Direction.DOWN, ERNO
			.Direction.LEFT, ERNO.Direction.BACK
		][a]
	};
	...
	ERNO.Direction.getDirectionByNormal = function() {
		...
		return function(b) {
			...
			a.y = Math.round(b.y);
			a.z = Math.round(b.z);
			return a.equals(ERNO.Direction.FRONT.normal) ? ERNO.Direction.FRONT : a.equals(ERNO.Direction
					.BACK.normal) ? ERNO.Direction.BACK : a.equals(ERNO.Direction.UP.normal) ? ERNO
				.Direction.UP : a.equals(ERNO.Direction.DOWN.normal) ? ERNO.Direction.DOWN : a.equals(ERNO
					.Direction.LEFT.normal) ? ERNO.Direction.LEFT : a.equals(ERNO.Direction.RIGHT.normal) ?
				ERNO.Direction.RIGHT : null
		}
	}();
	...
	ERNO.Direction.FRONT.setRelationships(ERNO.Direction.UP, ERNO.Direction.RIGHT, ERNO.Direction.DOWN, ERNO
		.Direction.LEFT, ERNO.Direction.BACK);
	ERNO.Direction.UP.setRelationships(ERNO.Direction.BACK, ERNO.Direction.RIGHT, ERNO.Direction.FRONT, ERNO
		.Direction.LEFT, ERNO.Direction.DOWN);
	ERNO.Direction.RIGHT.setRelationships(ERNO.Direction.UP, ERNO.Direction.BACK, ERNO.Direction.DOWN, ERNO
		.Direction.FRONT, ERNO.Direction.LEFT);
	ERNO.Direction.DOWN.setRelationships(ERNO.Direction.FRONT, ERNO.Direction.RIGHT, ERNO.Direction.BACK, ERNO
		.Direction.LEFT, ERNO.Direction.UP);
	ERNO.Direction.LEFT.setRelationships(ERNO.Direction.UP, ERNO.Direction.FRONT, ERNO.Direction.DOWN, ERNO
		.Direction.BACK, ERNO.Direction.RIGHT);
	ERNO.Direction.BACK.setRelationships(ERNO.Direction.UP, ERNO.Direction.LEFT, ERNO.Direction.DOWN, ERNO.Direction
		.RIGHT, ERNO.Direction.FRONT);
	...
	ERNO.Twist.prototype.set = function(a, b) {
		...
		if (void 0 !== c) {
			...
			var d = 0,
				e = "unwise";
			a === a.toUpperCase() ? (d = 1, e = "clockwise") : a === a.toLowerCase() && (d = -1, e =
				"anticlockwise");
			this.command = a;
			this.group = c;
			this.degrees = b;
	...
	ERNO.Twist.validate = function() {
				...
				if ("string" === typeof b)
					if (1 === b.length) a[c] = "number" === typeof d ? new ERNO.Twist(b, d) : new ERNO.Twist(b);
						...
						if (1 < b.length) {
							d = /(-?\d+|[XLMRYUEDZFSB])/gi;
							b = b.match(d);
							for (f = 0; f < b.length; f++) e = b[f], _.isNumeric(e) ? b[f] = +e : (d = b.slice(
								0, f), h = b.slice(f + 1), e = e.split(""), b = d.concat(e, h));
							d = a.slice(0, c);
							h = a.slice(c + 1);
							a = d.concat(b, h);
							c--
		...
		else b instanceof ERNO.Direction ? a[c] = b.initial : b instanceof Array ? (d = a.slice(0, c), h = a
			.slice(c + 1), a = d.concat(b, h)) : a.splice(c, 1), c--;
		return a
	};
	ERNO.Cubelet = function(a, b, c) {
		...
		void 0 === c && (c = [W, O, , , G]);
		this.faces = [];
		for (b = 0; 6 > b; b++) d = c[b] || ERNO.COLORLESS, this.faces[b] = {}, this.faces[b].id = b, this
			.faces[b].color =
			d, this.faces[b].normal = ERNO.Direction.getNameById(b), this.faces[b].isIntrovert = d === ERNO
			.COLORLESS, d !== ERNO.COLORLESS && a++;
		...
		this.colors = (this.faces[0].color ? this.faces[0].color.initial : "-") + (this.faces[1].color ? this
				.faces[1].color.initial : "-") + (this.faces[2].color ? this.faces[2].color.initial : "-") + (
				this.faces[3].color ?
				this.faces[3].color.initial : "-") + (this.faces[4].color ? this.faces[4].color.initial : "-") +
	...
	ERNO.extend(ERNO.Cubelet.prototype, {
		...
		hasColor: function(a) {
			...
			for (a = 0; 6 > a; a++)
				if (c = _.hexToRgb(this.faces[a].color.hex), c.r === d.r && c.g === d.g && c.b === d
					.b) {
					b = a;
					break
		...
		setRadius: function(a, b) {
			if (!1 === this.isTweening && (a = a || 0, void 0 === this.radius && (this.radius = 0), this
					.radius !== a)) {
				this.isTweening = !0;
				...
				(new TWEEN.Tween(d)).to({
					...
					this.position.set(this.addressX.multiply(this.size + d.radius) + 0.2, this
						.addressY.multiply(this.size + d.radius) + 0.2, this.addressZ
						.multiply(this.size + d.radius) +
						0.2);
					this.updateMatrix();
					...
					this.position.set(this.addressX.multiply(this.size + d.radius) + 0.2, this
						.addressY.multiply(this.size + d.radius) + 0.2, this.addressZ
						.multiply(this.size + d.radius) + 0.2);
					this.updateMatrix();
					this.matrixSlice.copy(this.matrix);
					this.radius = d.radius;
	...
	ERNO.extend(ERNO.Group.prototype, {
		...
		remove: function(a) {
			...
			for (var c = this.cubelets.length; 0 < c--;) this.cubelets[c] === a && this.cubelets.splice(
				c, 1);
			return this
		},
	...
	ERNO.Slice = function(a, b) {
		...
		this.rotateGroupMappingOnAxis = function() {
			...
			return function(m) {
				...
				for (var q = a.length, p; 0 < q--;) p = b.cubelets[a[q]], e.set(p.addressX, p.addressY,
						p.addressZ), f.copy(e), e.multiply(c).applyMatrix4(h), e.x = Math.round(e.x), e
					.y = Math.round(e.y), e.z = Math.round(e.z), e.add(f.multiply(this.axis)), e.add(d),
					e.y = 2 - e.y, e.z = 2 - e.z, b.cubelets[p.address] = s[9 * e.z + 3 * e.y + e.x];
	...
	ERNO.extend(ERNO.Slice.prototype, {
		...
		map: function(a, b) {
			...
			(c = this.hasType("center")) && 1 === c.cubelets.length ? (this.center = this.hasType(
					"center"), this.corners =
				new ERNO.Group(this.hasType("corner")), this.cross = new ERNO.Group(this.center,
					this.hasType("edge")), this.ex = new ERNO.Group(this.center, this.hasType(
					"corner"))) : this.centers = new ERNO.Group(this.hasType("center"));
			this.edges = new ERNO.Group(this.hasType("edge"));
			this.ring = new ERNO.Group(this.northWest, this.north, this.northEast, this.west, this.east,
		...
		set rotation(a) {
			if (this.ableToHideInternalFaces && 0 !== this.cube.isFlagged("showingIntroverts") && this
				.cube.hideInvisibleFaces) {
				var b = 0 !== a % (0.5 * Math.PI);
				this.invertedAxis.copy(this.axis).negate();
				b ? this.neighbour ? (this.showIntroverts(this.axis, !0), this.neighbour.showIntroverts(
					this.invertedAxis, !0)) : (this.cube.showIntroverts(this.axis, !0), this.cube
					.showIntroverts(this.invertedAxis, !0)) : this.neighbour ? (this.hideIntroverts(
					null, !0), this.neighbour.hideIntroverts(null,
					!0)) : this.cube.hideIntroverts(null, !0)
			}
			this.matrix.makeRotationAxis(this.axis, a);
			this.axis.rotation = a;
			a = this.indices.length;
			for (var c = new THREE.Matrix4; a--;) b = this.getCubelet(a), b.matrix.multiplyMatrices(this
					.matrix, b.matrixSlice), b.position.setFromMatrixPosition(b.matrix), b.scale
				.setFromMatrixScale(b.matrix), c.extractRotation(b.matrix), b.quaternion
				.setFromRotationMatrix(c)
		},
		get rotation() {
		...
		getLocation: function(a) {
			return a === this.origin ? "origin" : a === this.north ? "north" : a ===
				this.northEast ? "northEast" : a === this.east ? "east" : a === this.southEast ?
				"southEast" : a === this.south ? "south" : a === this.southWest ? "southWest" : a ===
				this.west ? "west" : a === this.northWest ? "northWest" : !1
		},
		isSolved: function(a) {
			if (a) {
				var b = {},
					...
					e = 0;
				a instanceof ERNO.Direction && (a = a.name);
				for (; 0 < d--;) c = this.getCubelet(d), c = c[a].color.name, void 0 === b[c] ? (b[c] =
					1, e++) : b[c]++;
				return 1 === e ? !0 : !1
			}
			console.warn(
	...
	ERNO.Fold = function(a, b) {
		this.map = [a.northWest[a.face].text, a.north[a.face].text, a.northEast[a.face].text, b.northWest[b
				.face].text, b.north[b.face].text, b.northEast[b.face].text, a.west[a.face].text, a.origin[a
				.face].text, a.east[a.face].text, b.west[b.face].text, b.origin[b.face].text, b.east[b.face]
			.text, a.southWest[a.face].text, a.south[a.face].text, a.southEast[a.face].text, b.southWest[b
				.face].text, b.south[b.face].text, b.southEast[b.face].text
		]
	};
	...
	ERNO.Projector = function() {
		return function(a, b) {
			...
			function d(d, f, l) {
				...
				t !== document && (t = t.ownerDocument.documentElement, k.left += window.pageXOffset - t
					.clientLeft, k.top += window.pageYOffset - t.clientTop);
				e = k;
			...
			return {
				getIntersection: function(b, c, e, f, g) {
					...
					if (null === c || null ===
						e) return null;
					d(b, c, e);
					return q.isIntersectionSphere(k) && null !== q.intersectBox(p, f) ? (g && (b = s ||
						new THREE.Vector3, b.copy(f).set(Math.round(b.x), Math.round(b.y), Math
							.round(b.z)).multiplyScalar(2 / a.size).set(b.x | 0, b.y | 0, b.z |
							0), g.setFromNormalAndCoplanarPoint(s, f)), f) : null
				},
				...
				getCubeletAtIntersection: function() {
					...
					return function(c) {
						b.copy(c).add(p.max).multiplyScalar(3 /
							a.size).set(Math.min(b.x | 0, 2), Math.min(3 - b.y | 0, 2), Math
							.min(3 - b.z | 0, 2));
	...
	ERNO.Interaction = function() {
		return function(a, b, c, d, e) {
			function f(e) {
				v.enabled && 2 !== e.button && (l = (e.touches && e.touches[0] || e).clientX, u = (e
					.touches && e.touches[0] || e).clientY, m.getIntersection(b, l, u, s, w) && (
					null !== e.touches && e.preventDefault(), 0 === a.isTweening() && (E =
						"undefined" !== typeof window && void 0 !== window.performance && void 0 !==
						window.performance.now ? window.performance.now() : Date.now(), v.active = !
						0, q = m.getCubeletAtIntersection(s), p = [a.slices[q.addressX + 1], a
							.slices[q.addressY + 4], a.slices[q.addressZ +
								7]
						], c.addEventListener("mousemove", h), c.addEventListener("touchmove", h), c
						.addEventListener("mouseup", g), c.addEventListener("touchcancel", g), c
						.addEventListener("touchend", g), c.removeEventListener("mousedown", f), c
						.removeEventListener("touchstart", f))))
			}

			function h(a) {
				v.active && (x.x = (a.touches && a.touches[0] || a).clientX, x.y = (a.touches && a.touches[
					0] || a).clientY);
			...
			function g(b) {
				var e = (b.touches && b.touches[0] || b).clientX,
					d = (b.touches && b.touches[0] ||
						b).clientY;
				v.active = !1;
				v.enabled && (e !== u || d !== u) && n && (null !== b.touches && b.preventDefault(), b = k
					.name[0].toUpperCase(), e = Math.round(C / Math.PI * 2) * Math.PI * 0.5, 0.3 < t
					.length() / (("undefined" !== typeof window && void 0 !== window.performance &&
							void 0 !== window.performance.now ? window.performance.now() : Date.now()) -
						E) && (e = Math.floor(C / Math.PI * 2) * Math.PI * 0.5, e += 0 < D.dot(t
						.normalize()) ? 0.5 * Math.PI : 0), a.twist(new ERNO.Twist(b, e
						.radiansToDegrees())));
				E = 0;
				x.x = void 0;
				x.y = void 0;
			...
			v.getIntersectionAt = function() {
				...
				return function(e, d) {
					return null === m.getIntersection(b, e, d, a, c) ? null : {
						...
						face: 1 === c.normal.x ? "RIGHT" : -1 === c.normal.x ? "LEFT" : 1 === c
							.normal.y ? "UP" : -1 === c.normal.y ? "DOWN" : 1 === c.normal.z ?
							"FRONT" : "BACK"
					}
				}
			}();
			v.update = function() {
				var c = x.x,
					e = x.y;
				v.enabled && v.active && void 0 !== c && void 0 != e && (l !== c || u !== e) && (m
					.getIntersectionOnPlane(b, c, e, w, A),
					t.subVectors(A, s), !n && 5 < t.length() && (k && (k.rotation = 0), n = !0, r
						.crossVectors(w.normal, t), c = Math.max(Math.abs(r.x), Math.abs(r.y), Math
							.abs(r.z)), r.x = r.x / c | 0, r.y = 1 === r.x ? 0 : r.y / c | 0, r.z =
						1 === r.x || 1 === r.y ? 0 : r.z / c | 0, k = p[Math.abs(3 * r.z + 2 * r.y +
							r.x) - 1], D.crossVectors(k.axis, w.normal).normalize()), n && (t
						.subVectors(A, s), C = D.dot(t) / a.size * v.dragSpeed), k && (k.rotation =
						C))
			};
			c.addEventListener("mousedown", f);
			c.addEventListener("touchstart", f);
			...
			c.addEventListener("mouseup", function(a) {
				var b = a.clientX;
				a = a.clientY;
				!n && Math.abs(Math.sqrt((b - y) * (b - y) + (a - z) * (a - z))) < 10 * (window
					.devicePixelratio || 1) && F(y, z)
			});
			...
			c.addEventListener("touchend", function(a) {
				...
				!n && Math.abs(Math.sqrt((b - y) * (b - y) + (c - z) *
						(c - z))) < 10 * (window.devicePixelratio || 1) && F(y, z) && a
					.preventDefault()
			});
			return v
		}
	...
	ERNO.Controls = function() {
		...
		return function(c, d, e) {
			function f(b) {
				n.enabled && 1 === b.which && null === A.getIntersection(d, b.pageX, b.pageY) && (p = a, k
					.multiplyScalar(0), w(b.pageX, b.pageY, l), u.copy(l), n.domElement
					.removeEventListener("mousedown", f), document.addEventListener("mousemove", h),
					document.addEventListener("mouseup", g))
			}

			function h(b) {
				n.enabled && (b.preventDefault(), p = a, w(b.pageX, b.pageY, l), k.subVectors(l, u), u.copy(
					l))
			}
			...
			function m(b) {
				n.enabled && null === A.getIntersection(d, b.touches[0].pageX, b.touches[0].pageY) && (p =
					a, k.multiplyScalar(0), w(b.touches[0].pageX, b.touches[0].pageY, l), u.copy(l), n
					.domElement.removeEventListener("touchstart", m), document.addEventListener(
						"touchend", q), document.addEventListener("touchmove", s))
			}

			function s(b) {
				n.enabled && (p = a, w(b.changedTouches[0].pageX, b.changedTouches[0].pageY, l), k
					.subVectors(l, u), u.copy(l))
			}
			...
			n.update = function() {
				var e = new THREE.Vector3,
					...
					g = new THREE.Matrix4;
				return function() {
					!1 !== n.enabled && -1 !== p && (e.set(k.y, -1 * k.x, 0).normalize(), g
						.getInverse(c.matrixWorld), g.multiply(d.matrixWorld), e
						.transformDirection(g), k.multiplyScalar(1 - Math.max(0, Math.min(1, n
							.damping))), f = k.length(), c.object3D.rotateOnAxis(e, -f * n
							.rotationSpeed), p === a ? p = -1 : p === b && 1E-4 <= f ? l.add(
							k) : p = -1)
				}
			}();
			n.domElement.addEventListener("mousedown", f);
	...
	ERNO.renderers.CSS3D = function(a, b) {
		...
		function f() {
			if (b.domElement.parentNode) {
				...
				!b.domElement.parentNode || b.domElement.clientWidth === a && b.domElement.clientHeight === c ||
					b.setSize(a, c);
				h.render(g, b.camera)
			}
	...
	ERNO.renderers.CSS3DCubelet = function() {
		return function(a) {
			...
			a.add(a.css3DObject);
			var b = a.size / 2,
				c = ["rotateX(   0deg ) translateZ( " + b + "px ) rotateZ(   0deg )",
			...
			a.faces.forEach(function(b) {
				...
				b.isIntrovert ? (b.element.classList.add("faceIntroverted"), b.element.appendChild(
					document.createElement("div"))) : (b.element.classList.add(
						"faceExtroverted"), f = document.createElement("div"), f.classList.add(
						"sticker"), f.classList.add(b.color.name),
					b.element.appendChild(f), a.isStickerCubelet && f.classList.add(
						"stickerLogo"), f = document.createElement("div"), f.classList.add(
						"text"), f.innerText = b.id, b.text = f, b.element.appendChild(f))
			});
			a.show();
			a.showIntroverts();
			a.showPlastics();
	...
	ERNO.renderers.CSS3DCubelet.methods = function() {
		...
		return {
			...
			showIntroverts: function() {
				...
				return function(f, h) {
					e = "";
					f && (d.getInverse(this.matrix), b.copy(f).transformDirection(d), e = 1 === Math
						.abs(Math.round(b.x)) ? ".axisX" : 1 === Math.round(Math.abs(b.y)) ?
						".axisY" : ".axisZ");
			...
			hideIntroverts: function() {
				...
				return function(f, h) {
					...
					f && (d.getInverse(this.matrix),
						a.copy(f).transformDirection(d), e = 1 === Math.abs(Math.round(a.x)) ?
						".axisX" : 1 === Math.round(Math.abs(a.y)) ? ".axisY" : ".axisZ");
	...
	ERNO.Cube = function(a) {
		...
		this.slices = [this.left, this.middle, this.right, this.down, this.equator, this.up, this.back, this
			.standing, this.front
		];
		var c = function(a) {
		...
		this.folds = [new ERNO.Fold(this.front, this.right), new ERNO.Fold(this.left, this.up), new ERNO.Fold(
			this.down, this.back)];
		this.setSize(400, 200);
		this.loop = this.loop.bind(this);
		requestAnimationFrame(this.loop);
		document.addEventListener("keypress", function(a) {
			"input" !== a.target.tagName.toLowerCase() && "textarea" !== a.target.tagName
				.toLowerCase() && !this.mouseInteraction.active && this.keyboardControlsEnabled &&
				(a = String.fromCharCode(a.which), 0 <= "XxRrMmLlYyUuEeDdZzFfSsBb".indexOf(a) && this
					.twist(a))
		}.bind(this))
	};
	...
	ERNO.extend(ERNO.Cube.prototype, {
		shuffle: function(a, b) {
			...
			for (this.historyQueue.empty(!0); 0 < a--;) {
				if (b) d.set(b[g]), g = (g + 1) % h;
					...
					for (f = c.split(""), d = (new ERNO.Twist).copy(e); d.equals(e);) d.set(f.splice(
						Math.floor(Math.random() * f.length), 1)[0]);
				d.isShuffle = !0;
				this.twist(d);
				e = d.getInverse()
			}
		...
		isSolved: function() {
			return this.front.isSolved(ERNO.Direction.FRONT) &&
				this.up.isSolved(ERNO.Direction.UP) && this.right.isSolved(ERNO.Direction.RIGHT) && this
				.down.isSolved(ERNO.Direction.DOWN) && this.left.isSolved(ERNO.Direction.LEFT) && this
				.back.isSolved(ERNO.Direction.BACK)
		},
		undo: function() {
			0 < this.twistQueue.history.length && (this.historyQueue.add(this.twistQueue.undo()
				.getInverse()), this.undoing = !0)
		},
		redo: function() {
			0 < this.twistQueue.future.length && (this.undoing = !0, this.historyQueue.empty(), this
				.historyQueue.add(this.twistQueue.redo()))
		},
		twist: function(a) {
			this.undoing &&
		...
		immediateTwist: function(a) {
			0.8 <= this.verbosity && console.log("Executing a twist command to rotate the " + a.group +
				" " + a.wise + " by", a.degrees, "degrees.");
			for (var b = this.slicesDictionary[a.command.toLowerCase()], c = (void 0 === a.degrees ?
					90 : a.degrees) * a.vector, d = c.degreesToRadians(), e = Math.abs(d - b
					.rotation) / (0.5 * Math.PI) * this.twistDuration, f = b.indices.length, h; 0 <
				f--;) b.getCubelet(f).isTweening = !0;
			(new TWEEN.Tween(b)).to({
				...
				b.rotation = d;
				b.axis.rotation = 0;
				for (f = b.indices.length; 0 < f--;) h = b.getCubelet(f), h.isTweening = !1, h
					.updateMatrix(), h.matrixSlice.copy(h.matrix);
				0 !== c && (b.rotateGroupMappingOnAxis(d), this.dispatchEvent(new CustomEvent(
					"onTwistComplete", {
						detail: {
							slice: b,
							twist: a
				...
				a === this.finalShuffle && (this.finalShuffle = null, this.dispatchEvent(
		...
		setText: function(a, b) {
			void 0 === b ? (this.folds[0].setText(a), this.folds[1].setText(a), this.folds[2].setText(
				a)) : _.isNumeric(b) && 0 <= b && 2 >= b && this.folds[b].setText(a)
		},
		...
		loop: function() {
			...
			return function() {
				requestAnimationFrame(this.loop);
				var b = "undefined" !== typeof window && void 0 !== window.performance && void 0 !==
					window.performance.now ? window.performance.now() : Date.now(),
					c = b - (a || b);
				a = b;
				this.paused || (this.time += c, TWEEN.update(this.time), this.autoRotate && (this
						.rotation.x += this.rotationDelta.x, this.rotation.y += this
						.rotationDelta.y, this.rotation.z += this.rotationDelta.z), this
					.isReady && 0 === this.isTweening() && (b = this.undoing ? this
						.historyQueue : this.twistQueue, 0 === b.future.length ?
						this.isSolving && window.solver ? this.isSolving = window.solver
						.consider(this) : !0 === this.taskQueue.isReady && (b = this.taskQueue
							.do(), b instanceof Function && b()) : (c = b.do(), "x" !== c
							.command.toLowerCase() && "y" !== c.command.toLowerCase() && "z" !==
							c.command.toLowerCase() && 0 !== c.degrees && (this.moveCounter +=
								this.undoing ? -1 : 1), (0 === c.degrees || c.isShuffle) && b
							.purge(c), this.immediateTwist(c))), this.mouseInteraction.enabled =
					this.mouseControlsEnabled && !this.finalShuffle, this.mouseInteraction
					.update(), this.controls.enabled =
					this.mouseControlsEnabled && !this.mouseInteraction.active, this.controls
					.update())
			}
		}()
	});
	...
	ERNO.Solver.prototype.consider = function(a) {
		if (void 0 === a) return console.warn(
			"A cube [Cube] argument must be specified for Solver.consider()."), !1;
		if (!1 === a instanceof ERNO.Cube) return console.warn(
			"The cube argument provided is not a valid Cube."), !1;
		a.isShuffling = !1;
		return a.isSolved() ? (ERNO.Solver.prototype.explain(
			"I\u2019ve found that the cube is already solved."), !1) : this.logic(a)
	};
