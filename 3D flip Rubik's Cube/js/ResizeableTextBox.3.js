...
ERNO.extend(ResizeableTextBox.prototype, {
	prepareText: function(text) {
		...
		if (words.length > 1) {
			...
			if (words[longWordIndex].length > this.maxWordLength) words[longWordIndex] = words[
				longWordIndex].substring(0, this.maxWordLength) + "...";
	...
	resize: function() {
		var fontSize = Math.min(this.max, Math.max(this.min, this.css3DObject.element.clientWidth / this
			.longWord.length * 1.3));
