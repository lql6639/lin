ERNO.Locked = function(cube, camera, domElement) {
	...
	axisDefined = false;
	var initialRotation = new THREE.Matrix4().makeRotationFromEuler(new THREE.Euler(Math.PI * 0.1, Math.PI * -0.25,
		0));
	...
	function onInteractStart(event) {
		if (api.enabled && projector.getIntersection(camera, (event.touches && event.touches[0] || event).pageX, (
			...
			if (projector.getIntersection(camera, x, y) === null) {
				if (cube.isTweening() === 0) {
					time = (typeof window !== 'undefined' && window.performance !== undefined && window.performance
	...
	function onInteractEnd(event) {
		...
		if (axisDefined) {
			...
			velocity = direction.length() / ((typeof window !== 'undefined' && window.performance !== undefined &&
